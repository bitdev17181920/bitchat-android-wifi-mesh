...
class MessageRouter private constructor(
    ...
    fun sendPrivate(content: String, toPeerID: String, recipientNickname: String, messageID: String) {
        ...
        val hasBle = mesh.getPeerInfo(toPeerID)?.isConnected == true
        val hasWifiMesh = mesh.isWiFiMeshActive()
        val hasEstablished = mesh.hasEstablishedSession(toPeerID)
        if ((hasBle || hasWifiMesh) && hasEstablished) {
            val via = if (hasWifiMesh && !hasBle) "wifi-mesh" else "mesh"
            Log.d(TAG, "Routing PM via $via to ${toPeerID} msg_id=${messageID.take(8)}…")
    ...
    fun sendReadReceipt(receipt: ReadReceipt, toPeerID: String) {
        val hasMeshPath = (mesh.getPeerInfo(toPeerID)?.isConnected == true) || mesh.isWiFiMeshActive()
        if (hasMeshPath && mesh.hasEstablishedSession(toPeerID)) {
    ...
    fun sendFavoriteNotification(toPeerID: String, isFavorite: Boolean) {
        if (mesh.getPeerInfo(toPeerID)?.isConnected == true || mesh.isWiFiMeshActive()) {
    ...
    fun flushOutboxFor(peerID: String) {
        ...
        while (iterator.hasNext()) {
            ...
            var hasMesh = (mesh.getPeerInfo(peerID)?.isConnected == true || mesh.isWiFiMeshActive()) && mesh.hasEstablishedSession(peerID)

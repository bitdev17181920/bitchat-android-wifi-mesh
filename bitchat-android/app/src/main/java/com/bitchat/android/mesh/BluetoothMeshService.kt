...
class BluetoothMeshService(private val context: Context) {
    ...
    private val wifiMeshTransport = com.bitchat.android.mesh.wifi.WiFiMeshTransport(context, myPeerID)
    private val transportManager = TransportManager().apply {
        addTransport(connectionManager)
        addTransport(wifiMeshTransport)
    }
    ...
    init {
        ...
        gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
            override fun sendPacket(packet: BitchatPacket) {
                transportManager.broadcastPacket(RoutedPacket(packet))
            }
            override fun sendPacketToPeer(peerID: String, packet: BitchatPacket) {
                transportManager.sendPacketToPeer(peerID, packet)
            }
    ...
    private fun setupDelegates() {
        ...
        securityManager.delegate = object : SecurityManagerDelegate {
            ...
            override fun sendHandshakeResponse(peerID: String, response: ByteArray) {
                ...
                transportManager.broadcastPacket(RoutedPacket(signedPacket))
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun relayPacket(routed: RoutedPacket) {
                transportManager.broadcastPacket(routed)
            }
            ...
            override fun onMessageReceived(message: BitchatMessage) {
                Log.d(TAG, "📬 onMessageReceived: sender=${message.sender}, content='${message.content?.take(50)}', isPrivate=${message.isPrivate}, channel=${message.channel}")
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun sendToPeer(peerID: String, routed: RoutedPacket): Boolean {
                return transportManager.sendToPeer(peerID, routed)
            }
        ...
        // WiFi mesh transport — feed incoming packets into the same processor
        wifiMeshTransport.onPacketReceived = { packet, senderID ->
            packetProcessor.processPacket(RoutedPacket(packet, senderID, "wifi-mesh"))
        }
    }
    ...
    fun startServices() {
        ...
        // Always start WiFi mesh transport (works independently of Bluetooth)
        wifiMeshTransport.start()
        Log.d(TAG, "WiFiMeshTransport started")
        ...
        if (connectionManager.startServices()) {
            ...
            Log.e(TAG, "Failed to start Bluetooth services, but WiFi mesh is active")
            // Still mark as active since WiFi mesh is running
            isActive = true
        }
    }
    ...
    fun stopServices() {
        ...
        serviceScope.launch {
            ...
            // Stop all components
            wifiMeshTransport.stop()
            Log.d(TAG, "WiFiMeshTransport stopped")
    ...
    fun sendFileBroadcast(file: com.bitchat.android.model.BitchatFilePacket) {
        ...
        serviceScope.launch {
            ...
            transportManager.broadcastPacket(RoutedPacket(signed, transferId = transferId))
    ...
    fun cancelFileTransfer(transferId: String): Boolean {
        return transportManager.cancelTransfer(transferId)
    }
    ...
    fun isWiFiMeshActive(): Boolean = wifiMeshTransport.isActive

    fun getWiFiMeshRelayHost(): String? = wifiMeshTransport.relayHost

    fun getWiFiMeshReachablePeers(): Set<String> = wifiMeshTransport.getReachablePeerIDs()

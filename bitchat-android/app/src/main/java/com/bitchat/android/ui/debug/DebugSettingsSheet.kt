...
fun DebugSettingsSheet(
    ...
    BitchatBottomSheet(
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            ...
            // WiFi mesh transport status
            item {
                val wifiActive = remember { meshService.isWiFiMeshActive() }
                val relayHost = remember { meshService.getWiFiMeshRelayHost() }
                val wifiPeers = remember { meshService.getWiFiMeshReachablePeers() }

                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Icon(Icons.Filled.Wifi, contentDescription = null, tint = if (wifiActive) Color(0xFF00C851) else Color(0xFF8E8E93))
                            Text("wifi mesh transport", fontFamily = FontFamily.Monospace, fontSize = 14.sp, fontWeight = FontWeight.Medium)
                        }
                        val statusColor = if (wifiActive) Color(0xFF00C851) else colorScheme.onSurface.copy(alpha = 0.6f)
                        Text(
                            if (wifiActive) "connected to relay" else "disconnected",
                            fontFamily = FontFamily.Monospace, fontSize = 11.sp, color = statusColor
                        )
                        if (relayHost != null) {
                            Text("relay: $relayHost:7275", fontFamily = FontFamily.Monospace, fontSize = 11.sp, color = colorScheme.onSurface.copy(alpha = 0.7f))
                        }
                        Text("peers via wifi: ${wifiPeers.size}", fontFamily = FontFamily.Monospace, fontSize = 11.sp, color = colorScheme.onSurface.copy(alpha = 0.7f))
                        if (wifiPeers.isNotEmpty()) {
                            wifiPeers.take(10).forEach { pid ->
                                Text("  • ${pid.take(12)}…", fontFamily = FontFamily.Monospace, fontSize = 10.sp, color = colorScheme.onSurface.copy(alpha = 0.6f))
                            }
                            if (wifiPeers.size > 10) {
                                Text("  … and ${wifiPeers.size - 10} more", fontFamily = FontFamily.Monospace, fontSize = 10.sp, color = colorScheme.onSurface.copy(alpha = 0.5f))
                            }
                        }
                    }
                }

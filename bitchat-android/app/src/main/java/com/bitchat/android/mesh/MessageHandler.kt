...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        if (peerInfo == null || !peerInfo.isVerifiedNickname) {
            if (com.bitchat.android.mesh.wifi.WiFiMeshConfig.DEBUG_SKIP_SIGNATURE_CHECK) {
                Log.w(TAG, "⚠️ DEBUG: Accepting message from unverified peer ${peerID.take(8)}")
            } else {
        ...
        try {
            ...
            Log.d(TAG, "📨 Plain text broadcast from ${peerID.take(8)}: '$textContent'")
            val message = BitchatMessage(
                sender = delegate?.getPeerNickname(peerID) ?: "mesh-${peerID.take(8)}",
                content = textContent,
                senderPeerID = peerID,
                timestamp = Date(packet.timestamp.toLong())
            )
            Log.d(TAG, "📨 Calling onMessageReceived for: sender=${message.sender}, content=${message.content}")
            delegate?.onMessageReceived(message)
        } catch (e: Exception) {
            Log.e(TAG, "Failed to process broadcast message: ${e.message}", e)
        }
    }
